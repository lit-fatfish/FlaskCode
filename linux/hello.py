...
def init_redis():
    ...
    # host = 'localhost'
    # pwd = ''
    redis_obj = redis.Redis(host=host, port=6379, password=pwd, db=0, decode_responses=True)
    return redis_obj
...
def status():
    ...
    datas = read_all_queue(r, "status_queue", -1)
    if datas:
        list = []
        for data in datas:
            temp = get_values(r, data)
...
def config_file():
    ...
    if request.method == "GET":
        ...
        rtml_list = read_all_queue(r,"status",-1)
        if rtml_list:
            for rtml in rtml_list:
                remove_key(r,rtml)
        r.delete("status_queue")
